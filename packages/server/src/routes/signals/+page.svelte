...
<div class="ledger text-xs [&_small]:!text-xs [&_p]:!text-xs [&_.kicker]:!text-xs [&_.summary_span]:!text-xs [&_.table-head]:!text-xs [&_.rows_code]:!text-xs [&_.rows_button]:!text-xs [&_.rows_button_small]:!text-xs [&_.rows_button_b]:!text-xs" data-theme={theme}>
    ...
    <main class="frame">
        <section class="ledger-head" data-reveal>
            ...
            <p>High-intent behavior deserves a separate reading surface. This register gathers captures, copies, scraping signatures, and other deliberate interactions without crowding the traffic overview.</p>
        </section>
        ...
        <section class="register" data-reveal>
            ...
            <div class="table-head"><span>Identity</span><span>Classification</span><span>Target surface</span><span>Volume</span><span>Shift</span><span>Last observed</span></div>
            <div class="rows">
                {#each filtered as event}
                    <button id={`event-${event.id}`} onclick={() => (selectedEvent = event)} aria-label={`Inspect ${event.label} from ${event.target}`}>
                        <code>{event.id}</code>
                        <p><i data-type={event.type}></i><span><strong>{event.label}</strong><small>{names[event.type]}</small></span></p>
                        <code>{event.target}</code>
                        <strong>{event.count.toLocaleString()}</strong>
                        <b class:negative={event.change < 0}>{event.change >= 0 ? "+" : ""}{event.change.toFixed(1)}%</b>
                        <small>{event.lastSeen}</small>
                    </button>
                {:else}
                    <div class="empty"><span>∅</span><h3>No classified signals yet.</h3><p>Events will appear here after the selected application begins sending them.</p></div>
                {/each}
            </div>
        </section>
        ...
        <section class="method" data-reveal>
            ...
            <div><strong>01</strong><h3>Observed</h3><p>A deliberate interaction enters the event stream with its surface and time.</p></div>
            <div><strong>02</strong><h3>Classified</h3><p>The signal is grouped as capture, copy, scraping, or contextual interaction.</p></div>
            <div><strong>03</strong><h3>Reviewed</h3><p>Volume and movement reveal which behavior needs attention from the team.</p></div>
        </section>
...
<style>
    ...
    .register{margin-top:46px}.register>header{display:flex;align-items:end;justify-content:space-between;padding:28px;box-shadow:0 1px var(--line)}h2{font-size:38px;line-height:1}.all-filter{padding:7px 10px;border:0;box-shadow:0 0 0 1px var(--line);color:var(--muted);background:transparent;font-family:"IBM Plex Mono",monospace;font-size:8px;text-transform:uppercase;cursor:pointer}.all-filter.active{color:var(--paper);background:var(--ink)}.table-head,.rows>button{display:grid;grid-template-columns:.7fr 1.3fr 1.4fr .45fr .45fr .6fr;align-items:center;gap:16px}.table-head{padding:11px 20px;box-shadow:0 1px var(--line);color:var(--muted);font-family:"IBM Plex Mono",monospace;font-size:7px;text-transform:uppercase}.rows>button{width:100%;min-height:70px;padding:12px 20px;border:0;box-shadow:0 1px var(--line);color:var(--ink);background:transparent;font-size:10px;text-align:left;cursor:pointer;transition:background 150ms ease}.rows>button:last-child{box-shadow:none}.rows>button:hover,.rows>button:focus-visible{background:color-mix(in srgb,var(--accent) 6%,transparent)}.rows code{overflow:hidden;color:var(--muted);font-family:"IBM Plex Mono",monospace;font-size:8px;text-overflow:ellipsis;white-space:nowrap}.rows>button>p{display:flex;align-items:center;gap:10px}.rows>button>p i{flex:0 0 auto;margin:0}.rows>button>p span{display:grid;gap:4px}.rows>button>p strong{font-weight:500}.rows>button>p small,.rows>button>small{color:var(--muted);font-family:"IBM Plex Mono",monospace;font-size:7px;text-transform:uppercase}.rows>button>strong{font-family:"Instrument Serif",Georgia,serif;font-size:22px;font-weight:400}.rows>button>b{color:var(--accent);font-family:"IBM Plex Mono",monospace;font-size:8px}.rows>button>b.negative{color:var(--muted)}.empty{display:grid;place-items:center;padding:74px 20px;text-align:center}.empty>span{color:var(--accent);font-family:"Instrument Serif",Georgia,serif;font-size:36px}.empty h3{margin:12px 0 6px;font-family:"Instrument Serif",Georgia,serif;font-size:26px;font-weight:400}.empty p{color:var(--muted);font-size:9px}
    ...
    @media(max-width:850px){.summary{grid-template-columns:1fr 1fr}.summary>div{grid-column:1/-1;border-right:0;border-bottom:1px solid var(--line)}.summary>button:nth-child(3),.summary>button:nth-child(5){border-right:0}.summary>button:nth-child(-n+3){border-bottom:1px solid var(--line)}.table-head{display:none}.rows>button{grid-template-columns:.6fr 1.4fr .5fr .5fr}.rows>button>code:nth-of-type(2),.rows>button>small{display:none}.method{grid-template-columns:1fr}.method>div{border-top:1px solid var(--line);border-left:0}}
    @media(max-width:620px){.frame,.nav-frame{width:min(100% - 28px,1160px)}.frame{padding-bottom:100px}.nav-frame>span{display:none}.ledger-head{grid-template-columns:1fr;gap:32px}.ledger-head>p{padding-left:16px}.summary{grid-template-columns:1fr}.summary>button{min-height:130px;border-right:0!important;border-bottom:1px solid var(--line)!important}.summary>button:last-child{border-bottom:0!important}.register>header{align-items:start;flex-direction:column;gap:20px}.rows>button{grid-template-columns:.7fr 1.3fr .5fr}.rows>button>b{display:none}}
