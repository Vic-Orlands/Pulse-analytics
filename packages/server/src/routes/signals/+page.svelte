<script lang="ts">
    ...
    const descriptions = {
        ...
        download: "File downloads from links and attachments",
    } as const;
...
<svelte:head>
    ...
    <main class="frame">
        ...
        <section class="summary" data-reveal>
            ...
            {#each ["screenshot", "copy", "scrape", "interaction", "outbound", "download"] as type}
...
<style>
    ...
    .summary{display:grid;grid-template-columns:repeat(3,1fr)}.summary>div,.summary>button{display:flex;min-width:0;min-height:140px;flex-direction:column;align-items:flex-start;padding:20px;border:0;color:var(--ink);background:transparent;text-align:left}.summary>div{grid-column:1/-1;min-height:120px;box-shadow:0 1px var(--line)}.summary>button{cursor:pointer;transition:background 160ms ease;box-shadow:-1px 0 var(--line),0 1px var(--line)}.summary>button:nth-child(3n+1){box-shadow:0 1px var(--line)}.summary>button:hover,.summary>button.active{background:color-mix(in srgb,var(--accent) 7%,transparent)}.summary span{color:var(--muted);font-family:"IBM Plex Mono",monospace;font-size:8px;text-transform:uppercase}.summary strong{margin:22px 0 10px;font-family:"Instrument Serif",Georgia,serif;font-size:36px;font-weight:400}.summary small{margin-top:auto;color:var(--muted);font-size:8px;line-height:1.5}.summary i,.register i{width:7px;height:7px;margin-bottom:12px;border-radius:50%;background:var(--accent)}i[data-type="copy"]{background:var(--comparison)}i[data-type="scrape"]{background:#d29a48}i[data-type="interaction"]{background:#7f8ed6}i[data-type="outbound"]{background:#5b8c7a}i[data-type="download"]{background:#8a6bb5}
